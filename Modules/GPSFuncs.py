...
def distCoordsComponentsSigned(cd1, cd2, altitude):

    earthRadii = radius((cd1[0] + cd2[0]) / 2) + altitude # Km EARTH RADII VARIES BASED ON LATITUDE - Average of two coords
    ...
    thetaLong = np.arccos(np.abs(dlon) / np.sqrt(dlon ** 2 + dlat ** 2))
    thetaLat = np.arcsin(dlat / np.sqrt(dlon ** 2 + dlat ** 2))

    if dlon < 0:
        x = -dist * cos(thetaLong)
    else:
        x = dist * cos(thetaLong)

    if dlat < 0:
        y = -dist*sin(thetaLong)
    else:
        y = dist*sin(thetaLong)
